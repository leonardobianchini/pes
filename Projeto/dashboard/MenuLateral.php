
<div id="sidebar"><a href="#" class="visible-phone"><i class="icon icon-home"></i> Página Inicial</a>
  <ul>
    <li class="active"><a href="index.php?pg=inicio"><i class="icon icon-home"></i> <span>Página Inicial</span></a> </li>
    <li> <a href="index.php?pg=freq"><i class="icon icon-signal"></i> <span>Frequência de Atividades</span></a> </li>
    <li> <a href="index.php?pg=equipe"><i class="icon icon-inbox"></i> <span>Equipes</span></a> </li>
    <li><a href="index.php?pg=ptor"><i class="icon icon-th"></i> <span>Próximos Eventos</span></a></li>
    <li><a href="index.php?pg=esporte"><i class="icon icon-fullscreen"></i> <span>Esportes</span></a></li>
    <li class="submenu"> <a href="#"><i class="icon icon-th-list"></i> <span>Inserir novo..</span> <span class="label label-important">3</span></a>
      <ul>
        <li><a href="index.php?pg=grupo">Grupo</a></li>
        <li><a href="index.php?pg=ambiente">Ambiente</a></li>
        <li><a href="index.php?pg=evento">Evento</a></li>
      </ul>
    </li>
    </li>
  </ul>
</div>
